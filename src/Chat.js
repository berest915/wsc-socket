...
// use server-time in case of users from different time zone
...
const Chat = () => {
	...
	useEffect(() => {
		if (roomId) {
			// point to the collection > find the roomId (the document)
			// pull the snapshot and setRoomName with the name of the document
			db.collection('rooms')
				.doc(roomId)
